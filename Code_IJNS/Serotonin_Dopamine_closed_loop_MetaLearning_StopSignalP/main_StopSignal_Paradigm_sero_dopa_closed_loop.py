...
from neurons import PPC,ACC_Qf,ACC,LPFC,Striatum,SNr,Thalamus,PMC
...
from mechanisms_meta_learning import rel_D1_beta,compute_TD_meta_learning,compute_forgetting
...
from neurotransmitters import dopamine, serotonin
...
#ACC action values
ACC_Q0 = ACC_Qf(params.alpha,params.Q0)
ACC_Q1 = ACC_Qf(params.alpha,params.Q0)
ACC_Q2 = ACC_Qf(params.alpha,params.Q0)
ACC_Q3 = ACC_Qf(params.alpha,params.Q0)
...
# parameters ML init
Sero  = serotonin(params.x0_sero,params.tau_sero,params.k_D1_S,params.k_D2_S,params.s0_sero, params.slope_sero)
DopaD1= dopamine(params.x0_dopaD1,params.tau_dopaD1,params.k_VTA_D1,params.s0_dopa, params.slope_dopa)
DopaD2=dopamine(params.x0_dopaD2,params.tau_dopaD2,params.k_VTA_D2,params.s0_dopa, params.slope_dopa)
...
#choose_func(4)
FUNC1= serotonin_to_dopamine_upreg
FUNC2=dopamine_to_serotonin_downreg
...
for n in range(params.niter-1): 
    ...
    DopaD1.update(Sero.x,params.deltaT,FUNC1)
    DopaD2.update(Sero.x,params.deltaT,FUNC1)
    Sero.update(DopaD1.x*TD,DopaD2.x,params.deltaT,FUNC2)
   
    vec_TD_DopaD1.append(DopaD1.x*TD)
    ...
    if sal[n]  == 1:
        ...
        if (cont_salient%2) != 0:
            ...
            if trial_type == 1:   
             ...
             print('Sero',Sero.x)
             print('DopaD1',DopaD1.x)
             print('DopaD2',DopaD2.x)
             print('count ppc: ', count_ppc) 
             [TD,flagSalient] = compute_TD_meta_learning(reward, QAct[action_conv_i][action_conv_j], QPrev[action_conv_i][action_conv_j], Sero.x, DopaD1.x)   
             print('delta reward:',delta_rew_out)             
             print('end trial')
             TD_memory.append(TD)
             
             # compute beta from D1
             beta.append(rel_D1_beta(params.a0_D1_to_beta,DopaD1.x*TD))
    ...
    for i in range(len(PPC_Neu)): 
        for j in range(len(PPC_Neu[0])):
            ...
            Striatum_Neu[i][j].update_s0(DopaD2.x, params.a0_D2_to_s0)
...
print(' level of serotonin:', np.mean(Sero.x))
print(' Level of D1:', np.mean(DopaD1.x))
print(' Level of D2:', np.mean(DopaD2.x))
print('##############################################')
...
print ('level of serotonin', np.mean(Sero.memory) )
print ('level of DopaD1', np.mean(DopaD1.memory))
print ('level of DopaD2', np.mean(DopaD2.memory))

Sero.memory=np.array (Sero.memory)
DopaD1.memory=np.array (DopaD1.memory)
DopaD2.memory=np.array (DopaD2.memory)

Sero_crucial=Sero.memory[np.where(time_crucial_trial)]
DopaD1_crucial=DopaD1.memory[np.where(time_crucial_trial)]
DopaD2_crucial=DopaD2.memory[np.where(time_crucial_trial)]
